...
### 3.自定义页面字体
首先获取到您想要更改的字体的字体文件（以woff2类型字体为首选，woff和ttf、otf其次），将字体文件复制到MyAI文件夹根目录下，并复制字体文件的名称（包括拓展名）。然后打开`index.html`，将第`23`行的`font_file`字段换成刚才复制到字体文件名。接着，您需要根据您的字体文件类型修改`23`行的`woff2`字段为：
